...
public class UbhRandomNearTarget : UbhBaseShot
{
	[Header("===== RandomShotNearTarget Settings =====")]
	...
	[FormerlySerializedAs("minRadiusNearTarget")]
	public float m_minRadiusNearTarget = 1;
	[FormerlySerializedAs("maxRadiusNearTarget")]
	public float m_maxRadiusNearTarget = 2;
	...
	protected virtual void Update()
	{
		...
		if (m_evenlyDistribute)
		{
			...
			angleMove = Random.Range(minAngle + (quarterAngle * quarterIndex), minAngle + (quarterAngle * (quarterIndex + 1f)));
		}
		else
		{
			angleMove = Random.Range(minAngle, maxAngle);
		}
		AimTarget();

		if (m_targetTransform != null)
		{
			Vector2 newpos = m_targetTransform.position;
			newpos += RotateBy(Vector2.up, angleMove, false) * Random.Range(m_minRadiusNearTarget, m_maxRadiusNearTarget);
